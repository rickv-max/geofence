...
export default function App() {
  ...
  // ==========================================
  // Cek apakah ada data wilayah yang tersimpan offline di memori HP
  const loadSavedData = () => {
    try {
      const saved = localStorage.getItem('bps_geofence_offline_data');
  ...
  // Jika ada data offline, langsung masuk ke mode tracking
  ...
  const [isInside, setIsInside] = useState(null); // null = menunggu gps, true = didalam, false = diluar
  const [gpsStatus, setGpsStatus] = useState('Menunggu Izin GPS...');
  ...
  // Monitor status koneksi internet untuk UI
  useEffect(() => {
  ...
  // ==========================================
  // PENCARIAN & AUTOCOMPLETE (ONLINE ONLY)
  // ==========================================
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (searchQuery.trim().length >= 3) {
        if(navigator.onLine) {
          ...
          setSearchError("Anda sedang Offline. Sambungkan internet untuk mencari wilayah baru.");
        }
      } else {
        setSuggestions([]);
        setSearchError(null);
  ...
  const fetchSuggestions = async (query) => {
    ...
    setSearchError(null);
    try {
      // Prioritaskan Polygon / Batas Wilayah
      ...
      if (data && data.length > 0) {
        // HANYA TERIMA HASIL YANG MEMILIKI POLIGON (Garis Batas Asli)
        ...
        if (validPolygons.length > 0) {
          ...
          setSearchError("Batas wilayah (Polygon) untuk area ini belum dipetakan di OpenStreetMap. Coba area yang lebih luas (Misal: Kecamatan).");
          setSuggestions([]);
        }
      } else {
        setSuggestions([]);
      ...
      setSearchError("Gagal mengambil data dari satelit peta.");
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleSelectLocation = (item) => {
    ...
    // SIMPAN KE OFFLINE STORAGE (HP INTERNAL)
    localStorage.setItem('bps_geofence_offline_data', JSON.stringify(item));
  ...
  // ==========================================
  useEffect(() => {
    if (appMode === 'tracking' && mapContainerRef.current) {
      if (!mapRef.current) {
      ...
      if (targetData && targetData.geojson) {
        ...
        const polygonStyle = {
          color: '#DC2626', // Merah yang sama seperti Google Maps
          weight: 3,
          opacity: 0.9,
          fillColor: '#DC2626', 
          fillOpacity: 0.08, // Sangat transparan agar jalan terlihat
          dashArray: '5, 8'
        };
  ...
  const startLiveTracking = () => {
    ...
    setGpsStatus('Menghubungkan ke Satelit GPS...');
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (accuracy <= 20) setGpsStatus('GPS Akurasi Tinggi (Lock)');
        else if (accuracy <= 80) setGpsStatus('GPS Akurasi Sedang');
        else setGpsStatus('GPS Akurasi Rendah (Cari ruang terbuka)');

        updateMapMarker(newLocation);
        checkGeofence(latitude, longitude);
      },
      (error) => {
        setIsInside(null); // Reset status agar tidak "Menghitung..." selamanya
        setLocation(null);
        if(error.code === error.PERMISSION_DENIED) {
          setGpsStatus('Akses GPS Ditolak! Izinkan di Pengaturan HP Anda.');
        } else if (error.code === error.POSITION_UNAVAILABLE) {
          setGpsStatus('Sinyal GPS Hilang/Tidak Tersedia.');
        } else {
  ...
  const centerToMe = () => {
    if(mapRef.current && location) {
      mapRef.current.setView([location.lat, location.lng], 18, { animate: true }); // Zoom lebih dekat (18)
    }
  };

  const resetApp = () => {
    // Jangan hapus data di localStorage agar bisa dipakai lagi nanti, kecuali user benar2 ganti
    if (watchIdRef.current) navigator.geolocation.clearWatch(watchIdRef.current);
  ...
  const deleteOfflineData = () => {
    localStorage.removeItem('bps_geofence_offline_data');
    setTargetData(null);
    setSearchQuery('');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-[#005A9C] to-[#003F6E] relative overflow-hidden select-none">
        ...
        <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl p-7 relative z-10 flex flex-col min-h-[480px]">
          ...
          {/* Jika ada data tersimpan, tawarkan untuk langsung masuk */}
          {targetData && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <span className="inline-flex items-center gap-1 mt-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 uppercase">
                          <CheckCircle2 className="w-3 h-3" /> Area Akurat (Poligon)
                        </span>
                      </div>
                    </li>
                  )
  ...
  // Tentukan state UI berdasarkan ketersediaan lokasi
  ...
  return (
    <div className="relative w-full h-screen bg-[#e5e5e5] overflow-hidden select-none">
      ...
      <style>{`
        ...
        /* Fallback background jika peta tidak muat karena offline */
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none z-10 flex flex-col justify-between">
        ...
        <div className="p-4 pointer-events-auto mt-2">
          ...
          {/* Indikator Offline Jika Tidak Ada Sinyal Saat Tracking */}
          {isOfflineMode && (
            <div className="mx-auto mt-2 bg-gray-800/90 backdrop-blur-sm text-white text-[10px] font-bold px-3 py-1.5 rounded-full inline-flex items-center gap-1.5 shadow-md absolute left-1/2 -translate-x-1/2 whitespace-nowrap">
               <DatabaseBackup className="w-3 h-3 text-emerald-400" /> Menggunakan Data Tersimpan (Offline)
            </div>
          )}
        </div>
        ...
        <div className="absolute bottom-[240px] right-4 flex flex-col gap-3 pointer-events-auto">
          <button 
            ...
            className="w-12 h-12 bg-white rounded-full shadow-[0_10px_30px_-10px_rgba(0,0,0,0.3)] flex items-center justify-center text-[#DC2626] hover:bg-gray-50 focus:outline-none"
            title="Lihat Seluruh Area Batas"
          >
          ...
          <button 
            ...
            title="Fokus Lokasi Saya (GPS)"
          >
            <Crosshair className="w-6 h-6" />
          </button>
        </div>

        {/* BOTTOM SHEET STATUS INFO */}
        <div className="bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.1)] rounded-t-[2rem] p-6 pointer-events-auto border-t border-gray-100 pb-8 mt-auto">
          ...
          <div className={`p-4 rounded-2xl flex items-center gap-4 transition-colors duration-300 shadow-sm border ${
            ...
            !isGpsLocked ? 'bg-orange-50 border-orange-100' : // Error / Ditolak
            ...
            <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-inner shrink-0 ${
              ...
              {showLoading ? <Loader2 className="w-7 h-7 animate-spin" /> :
               !isGpsLocked ? <MapPinOff className="w-7 h-7" /> :
            ...
            <div className="flex-1">
              <p className="text-[11px] font-bold uppercase tracking-wider mb-0.5 text-gray-500">
                Status Koordinat Realtime
              </p>
              <h3 className={`text-[18px] font-bold leading-tight ${
                showLoading ? 'text-gray-700' :
                !isGpsLocked ? 'text-orange-700' :
                isInside === true ? 'text-emerald-700' : 
                'text-red-700'
              }`}>
                {showLoading ? 'Menunggu Lokasi Anda...' : 
                 !isGpsLocked ? 'Lokasi Belum Ditemukan' :
                 isInside === true ? 'Posisi DI DALAM Batas' : 
                 'Posisi DI LUAR Batas'}
              </h3>
            </div>
          </div>
          ...
          {/* Tombol Manual Restart GPS jika error */}
          {(!isGpsLocked && !showLoading) && (
              <button 
                onClick={startLiveTracking}
                className="mt-4 w-full bg-orange-100 text-orange-700 hover:bg-orange-200 py-2.5 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-2"
              >
